...
def get_bmp_transparent_color(file_path):
    ...
    try:
        ...
        w, h = img.size
        # Bottom-left pixel is the transparent color by convention
        transparent_color = img.getpixel((0, h - 1))
...
def image_to_argb_array(file_path):
    ...
    if original_mode in ('RGB', 'P'):
        # Get transparent color from bottom-left pixel (MQL5 BMP convention)
