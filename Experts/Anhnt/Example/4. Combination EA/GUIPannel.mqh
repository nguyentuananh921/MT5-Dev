...
#define CGUIPANNEL_MQH_IMPLEMENTATION
 ...
 //Create GUI controls
    ...
    bool CGUIPannel::UpdateStatusBar(void)
     {
      ...
      double profit_val  = PositionsFloatingProfitTotal("");
      ...
      bool any_changed = false;
        if(new_deposit != s_deposit)
          {
            int img = (s_deposit == "") ? 2 :
                      (deposit_val > s_deposit_val) ? 0 :
                      (deposit_val < s_deposit_val) ? 1 : 2;
        ...
        if(new_profit != s_profit)
          {
            int img = (s_profit == "") ? 2 :
                      (profit_val > s_profit_val) ? 0 :
                      (profit_val < s_profit_val) ? 1 : 2;
            color clr = (profit_val > 0) ? clrGreen :
                        (profit_val < 0) ? clrRed : clrBlack;
  ...
  //For Symbol Information Table at tab Symbol Info
   ...
   bool CGUIPannel::SetValuesToSymbolInfoTable()
    {
      ...
      if(s_prev_total != total)
        {
          s_prev_total = total;
          ::ArrayResize(s_sym,     total); ::ArrayResize(s_bid,     total);
          ::ArrayResize(s_ask,     total); ::ArrayResize(s_spread,  total);
          ::ArrayResize(s_time,    total); ::ArrayResize(s_bid_val, total);
   ...
   //Clear symbol information table and show a message when thre are no symbols to display or an error occurs
