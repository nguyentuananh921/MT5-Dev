  ...
  int CGUIPannel::LoadIndicatorsFromJson(const string filename)
   {
      ...
      // --- Tầng 2 (GUI) phải đồng nhất với Tầng 1 (PureData) vừa nạp thêm indicator.
      // --- m_indicator_table tự quét lại m_indicators_timeseries + trạng thái ChartIndicatorName
      // --- hiện tại trên Tầng 3, nên chỉ cần gọi lại là đủ — không cần biết JSON nạp gì.
      if(applied > 0 && m_gui_created)
         SetValuesToIndicatorTable();
