 ...
 void CGUIPannel::LoadIndicatorTemplateSettingFromJSON(void)
  {
   ...
   // --- Startup = "every Series is new" (Anhnt, 2026-08-19) - reuse the SAME call CHARTCHANGE uses
   // --- (AddAllIndicatorsToNewSeries) once per Series LoadSymbolTFSettingFromJSON already created,
   // --- instead of a separate startup-only "Apply" path. .type_enum/.raw_params[] are already
   // --- populated (see above), so Layer 1 never touches text/schema/catalog at all.
    for(int i = 0; i < ArraySize(m_symbol_tf_Setting); i++)
