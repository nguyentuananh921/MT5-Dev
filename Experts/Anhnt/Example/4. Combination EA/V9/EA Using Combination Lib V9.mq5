 ...
 void OnTick(void)
  {
    ...
    // --- MUST run before mGUIPannel.OnTickEvent() (Anhnt, 2026-08-16): this is what actually
    // --- populates m_BarTimeSeriesCollection's new-bar event queue for the current symbol
    // --- (CBarSeriesDE::IsNewBar() is a consume-once detector - whoever calls it first "eats"
    // --- the flag). PlaySoundCloseBar() (inside mGUIPannel.OnTickEvent()) used to run first and
    // --- silently starve Layer 1's own event queue every tick - see BugNote for the full trace.
