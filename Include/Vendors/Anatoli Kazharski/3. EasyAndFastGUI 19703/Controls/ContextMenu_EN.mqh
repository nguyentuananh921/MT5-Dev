...
private:
   ...
   // (1) Text, (2) shortcut to available item, (3) shortcut to locked item
   string            m_text[];
...
// --- Handling a click event on a menu item
   ...
   for(int i=0; i<items_total; i++)
     {
      ...
      if(m_items[i].TypeMenuItem()==MI_HAS_CONTEXT_MENU && m_items[i].IsPressed())
        {
         // --- If the focus is on the context menu, but not on this item
