...
*/
#include <Vendors/Anhnt/CFramework/Defines/Defines.mqh>  // constants MARKER_START_DATA, CELL_WIDTH_IN_CHARS
#include <Vendors/Anhnt/CFramework/Defines/Enums.mqh>    // enums ENUM_OBJECT_TYPE, ENUM_DATATYPE

#include "..\ListObj.mqh"         // CTableListObj
...
//--- Move the current caption to the specified position in the list
   if(caption==NULL || !this.m_list_captions.MoveToIndex(caption_index,index_to))
      return false;
...
string CTableHeader::Description(void)
  {
   return(::StringFormat("%s: Captions total: %u",
                         TypeDescription((ENUM_TABLE_OBJECT_TYPE)this.Type()),this.ColumnsTotal()));
  }
  */
//+------------------------------------------------------------------+
