...
//+------------------------------------------------------------------+
#ifndef __PICTURESSLIDER_MQH__
#define __PICTURESSLIDER_MQH__
 #include "..\Element.mqh"
 #include "Picture.mqh"
 #include "Button.mqh"
 #include "ButtonsGroup.mqh"
 
 // --- Default picture #resource "\\Icons\\bmp64\\no_image.bmp" //Change to uint
 ...
 #define CPICTURESSLIDER_MQH_IMPLEMENTATION
   ...
   bool CPicturesSlider::CreatePictures(void)
   {
      ...
      if(pictures_total<1)
      {
         ::Print(__FUNCTION__," > This method must be called when there is at least one picture in the group! Use CPicturesSlider::AddPicture()");
         return(false);
      }
      ...
      for(int i=0; i<pictures_total; i++)
      {
         ...
         m_pictures[i].IconFile(m_image_resource_index[i]);
         m_pictures[i].IconFileLocked(m_image_resource_index[i]);
   ...
   void CPicturesSlider::AddPicture(const uint resource_index=0)
    {
   // --- Increase the size of the arrays by one element
      int array_size=::ArraySize(m_pictures);
      int new_size=array_size+1;
      ::ArrayResize(m_pictures,new_size);
      ::ArrayResize(m_image_resource_index,new_size);
      // --- Save the values ​​of the passed parameters
      //m_image_resource_index[array_size]=(file_path=="")? m_default_index : file_path;
      m_image_resource_index[array_size]=(resource_index==0)? m_default_index : resource_index;
    }
   //+------------------------------------------------------------------+
   ...
   //+------------------------------------------------------------------+
 #endif // CPICTURESSLIDER_MQH_IMPLEMENTATION
#endif // __PICTURESSLIDER_MQH__
