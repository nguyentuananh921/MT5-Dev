...
public:
  ...
  CScrollV *GetContentScrollVPointer(void) {
    return (::GetPointer(m_content_scrollv));
  }
  ...
  void NavigatorMode(const ENUM_FILE_NAVIGATOR_MODE mode) {
    m_file_navigator_mode = mode;
  }
  ...
  string SelectedItemFileName(void) const {
    return (m_selected_item_file_name);
  }
  ...
  if (m_t_is_folder[m_selected_item_index]) {
    ...
    if (m_selected_content_item_index > 0) {
      ...
      if (!m_t_is_folder[m_c_tree_list_index[m_selected_content_item_index]])
        m_selected_item_file_name =
            m_c_item_text[m_selected_content_item_index];
    }
  }
...
bool CTreeView::OnClickItemArrow(const string clicked_object, const int id,
  ...
  if (x < m_items[list_index].ArrowXGap() ||
      x > m_items[list_index].ArrowXGap() + 16)
    return (false);
