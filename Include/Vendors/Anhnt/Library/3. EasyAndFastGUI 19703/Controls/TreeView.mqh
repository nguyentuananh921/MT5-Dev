...
private:
   ...
   // If a file is not selected in the list, then this field should contain the empty string "".
...
public:
   ...
   // --- (1) File navigator mode, (2) Backlight mode when hovering the mouse cursor,
   // (3) mode for changing the width of lists, (4) mode of tab items
   ...
   // --- Number of items (1) in the tree list, (2) in the contents list and (3) visible number of items
   ...
   // --- (1) Selects an item by index and (2) returns the index of the selected item, (3) returns the file name
...
private:
 ...
 #define CTREEVIEW_MQH_IMPLEMENTATION
   ...
   {
   // --- Exit if the width of the content area does not need to be changed or tabbed item mode is enabled
   ...
   for(int i=0; i<total; i++)
      ...
      if(!m_t_is_folder[i])
         continue;
      // --- If (1) the index of the shared list is the same as the index of the shared list of the previous node and
      // (2) the index of the local list item is the same as the index of the previous node and
      // (3) the sequence of node levels is observed
   ...
   {
   // --- Exit if (1) the content of the item does not need to be shown or (2) the item is hidden
   ...
   // --- Exit if (1) outside the element area or (2) content area width change mode is activated
   ...
   {
   // --- Exit, (1) if the width of the content area does not need to be changed or
   // (2) tab item mode is enabled or (3) the scroll bar is active
  ...
  void CTreeView::UpdateXSize(const int x)
   {
      ...
      // --- Calculate and set the width for items in the tree list, taking into account the scroll bar
  ...
  void CTreeView::FormTreeList(void)
   {
      ...
      for(int nl=m_min_node_level; nl<=m_max_node_level && !end_list; nl++)
         ...
         for(int i=0; i<items_total && !::IsStopped(); i++)
            ...
            // --- If (1) this is not our node or (2) the sequence of local item indices is not respected,
            // let's move on to the next one
            if(nl!=m_t_node_level[i] || m_t_item_index[i]<=l_item_index[nl])
               continue;
            // --- Let's move on to the next point if (1) is not currently in the root directory and
            // (2) the total index of the list of the previous node is not equal to the same one in memory
            if(nl>m_min_node_level && m_t_prev_node_list_index[i]!=l_prev_node_list_index[nl])
               continue;
            // --- Remember the local index of the item if the next one is not less than the size of the local list
            ...
            // --- If you have not yet reached the last item in the root directory
               ...
               while(true)
                  ...
                  l_item_index[nl]           =-1;
                  l_items_total[nl]          =-1;
                  // --- Decrease the node counter until equality in the number of items in local lists is maintained
                  // or did not reach the root directory
  ...
  void CTreeView::FormContentList(void)
   {
      ...
      for(int i=0; i<items_total; i++)
         ...
         if(m_t_node_level[i]==m_t_node_level[li]+1 && 
            ...
            // --- Let's save the text of the item and the general index of the tree list in arrays
      ...
      if(cd_items_total>0)
         ...
         for(int i=0; i<c_items_total; i++)
            ...
            if(m_c_item_text[i]==m_cd_item_text[c] && 
               ...
               {
               // --- Let's save the general index of the content list and move on to the next one
