...
#define __ELEMENTBASE_MQH__ 
  ...
  class CVisualHint; // Forward declaration 
 ...
 #define CELEMENTBASE_IMPLEMENTATION
   ...
   string CElementBase::Description(void)
    {
      //--- 1. Get unified base info: "Element Type: Name (ID n)"
      string baseDesc = CBaseObj::Description();
      
      //--- 2. Format Graphic info (BG/FG) and Group
      string extraInfo = ::StringFormat("(BG: %s, FG: %s), Group: %d", 
                                       this.NameBG(), this.NameFG(), this.Group());
      
      //--- 3. Format detailed Area info
      string area = ::StringFormat("Area: [X %d, Y %d, W %d, H %d, R %d, B %d]", 
                                 this.X(), this.Y(), this.Width(), this.Height(), 
                                 this.Right(), this.Bottom());
      
      //--- 4. Final combination
      return ::StringFormat("%s %s, %s", baseDesc, extraInfo, area);
      //---------------------------------
      // string nm=this.Name();
   ...
   bool CElementBase::AddHintToList(CVisualHint *obj)
    {
     ...
     // --- If such element is not in the list,
      //if(this.m_list_hints.Search(obj)==NULL) //Modify to fix Include 
      //if(this.m_list_hints.Search((CObject*)obj) == NULL)
      if(this.m_list_hints.Search((CObject*)((void*)obj)) == NULL)
      {
         // --- return the list to its original sorting and return the result of adding it to the list
         this.m_list_hints.Sort(sort_mode);
         //return(this.m_list_hints.Add(obj)>-1);//Modify here to fix Include
         return(this.m_list_hints.Add((CObject*)((void*)obj)) > -1);
      }
