...
//+------------------------------------------------------------------+
#ifndef __CONTEXTMENU_MQH__
#define __CONTEXTMENU_MQH__
   #include "..\Element.mqh"
 ...
 class CContextMenu : public CElement
  {
   private:
     ...
     // (1) Text, (2) available item shortcut, (3) locked item shortcut
      string            m_text[];
      uint              m_image_resource_index_on[];  //m_path_bmp_on[];
      uint              m_image_resource_index_off[]; //m_path_bmp_off[];
 ...
 #define CCONTEXTMENU_MQH_IMPLEMENTATION
      ...
      for(int i=0; i<items_total; i++)
       {
        ...
        // --- Set properties
            ...
            m_items[i].IconFile(m_image_resource_index_on[i]);
            m_items[i].IconFileLocked(m_image_resource_index_off[i]);
            m_items[i].IconFilePressed(m_image_resource_index_on[i]);
            m_items[i].IconFilePressedLocked(m_image_resource_index_off[i]);
   ...
   // | Adds a menu item |
   //+------------------------------------------------------------------+
   void CContextMenu::AddItem(const string text,const uint resource_index_on,const uint resource_index_off,const ENUM_TYPE_MENU_ITEM type)
     {
   // --- Increase the size of the arrays by one element
      int array_size=::ArraySize(m_items);
      ::ArrayResize(m_items,array_size+1);
      ::ArrayResize(m_text,array_size+1);
      ::ArrayResize(m_image_resource_index_on,array_size+1);
      ::ArrayResize(m_image_resource_index_off,array_size+1);
   // --- Save the values ​​of the passed parameters
      m_text[array_size]=text;
      m_image_resource_index_on[array_size]  =resource_index_on;
      m_image_resource_index_off[array_size] =resource_index_off;
   ...
   // --- Freeing element arrays
      ...
      ::ArrayFree(m_text);
      ::ArrayFree(m_image_resource_index_on);
      ::ArrayFree(m_image_resource_index_off);
