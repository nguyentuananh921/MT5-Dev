 ...
 #define CFILENAVIGATOR_MQH_IMPLEMENTATION
   ...
   // --- Quit if there is no pointer to the main element
      if(!CElement::CheckMainPointer())
         return(false);
   // --- Scan the terminal file system and enter data into arrays
      FillArraysData();
      ...
      for(int i=0; i<items_total; i++)
        {
         ...
         uint icon_path=(m_g_is_folder[i])? m_folder_icon : m_file_icon;
   ...
   //+------------------------------------------------------------------+
   // | Handling the event of selecting a new path in a tree list |
   //+------------------------------------------------------------------+
   ...
   // --- If this is a local terminal folder
      else if(::StringFind(path,"MQL5\\Files\\",0)>-1)
        {
         // --- Get the address of the data in the local terminal folder
   ...
   // --- Counters of (1) general indexes, (2) node levels, (3) local indexes
      ...
      int node_level =0;
      int item_index =0;
   // --- If you need to display both directories (Shared (0)/Local (1))
      int begin=0,end=1;
   // --- If you need to display the contents of only the local directory
      ...
      for(int root_index=begin; root_index<=end; root_index++)
        {
         ...
         // ---Increase the counters of general indexes and node levels
         list_index++;
         node_level++;
         // --- Increase the size of the arrays by one element (relative to the node level)
         AuxiliaryArraysResize(node_level);
         // --- Initializing the first elements for the terminal's local folder directory
         if(root_index>0)
           {
      ...
      while(!::IsStopped())
        {
         ...
         if(node_level==1 && item_index>=m_l_item_total[node_level])
            break;
         // --- If you reach the end of the list in any node except the root
   ...
   //+------------------------------------------------------------------+
 #endif // CFILENAVIGATOR_MQH_IMPLEMENTATION
#endif // __FILENAVIGATOR_MQH__
