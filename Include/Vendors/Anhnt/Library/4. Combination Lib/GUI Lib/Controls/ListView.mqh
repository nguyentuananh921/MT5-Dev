...
#define CLISTVIEW_MQH
 ...
 bool CListView::CreateList(void)
  {
   ...
   // ---Create an object
    ...
    {
      ::Print(__FUNCTION__," > Failed to create a canvas for drawing the list: ",::GetLastError());
      return(false);
    }
   // --- Attach to chart
    if(!m_listview.Attach(m_chart_id,name,COLOR_FORMAT_ARGB_NORMALIZE))
     {
      ::Print(__FUNCTION__," > Failed to attach the drawing canvas to the chart: ",::GetLastError());
      return(false);
     }
   // --- Properties
 ...
 //+------------------------------------------------------------------+
 // | Getting the state of a checkbox at a specified index |
 //+------------------------------------------------------------------+
 ...
 void CListView::Rebuilding(const int items_total,const bool redraw=false)
  {
   // Reset stale hover/focus indexes from whatever list was here before - otherwise a later
   // mouse-move redraw can index m_items[] with an index only valid for the OLD (longer) list.
    m_item_index_focus      = WRONG_VALUE;
    m_prev_selected_item    = WRONG_VALUE;
    m_prev_item_index_focus = WRONG_VALUE;    
   // --- Set size to zero
 ...
 //+------------------------------------------------------------------+
 // | Calculation taking into account the latest changes              |
 // | and changing the list size                                      |
 //+------------------------------------------------------------------+
 ...
 //+------------------------------------------------------------------+
 // | Makes a copy of the specified item (source)                     |
 // | to a new location (dest.)                                       |
 //+------------------------------------------------------------------+
 ...
 //+------------------------------------------------------------------+
 // | Calculation of the Y-coordinate of the scroll bar               |
