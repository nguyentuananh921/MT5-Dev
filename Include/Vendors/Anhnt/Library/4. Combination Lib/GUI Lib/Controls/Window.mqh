...
//+------------------------------------------------------------------+
#ifndef __WINDOW_MQH__
#define __WINDOW_MQH__
...
#define CWINDOW_IMPLEMENTATION
  ...
  CWindow::CWindow(void) : m_right_limit(0),
                          m_caption_height(WINDOW_CAPTION_HEIGHT),//WINDOW_CAPTION_HEIGHT=22
  ...
  bool CWindow::CreateWindow(const long chart_id,const int subwin,const string caption_text,const int x,const int y)
   {
    ...
    // --- Quit if identifier is not defined
     if(CElementBase::Id()==WRONG_VALUE)
      {
        ::Print(__FUNCTION__," > Before creating a window, its pointer must be saved in the database: CWndContainer::AddWindow(CWindow &object)");
        return(false);
      }
    ...
    for(int b=0; b<buttons_total; b++)
      {
        ...
        // Setting the same Properties for 4 button
          ...
          button_obj.IconXGap(WINDOW_BUTTON_ICON_GAP); //WINDOW_BUTTON_ICON_GAP=2
          button_obj.IconYGap(WINDOW_BUTTON_ICON_GAP); //WINDOW_BUTTON_ICON_GAP=2
  ...
  //+------------------------------------------------------------------+
 #endif // CWINDOW_IMPLEMENTATION
#endif // __WINDOW_MQH__
