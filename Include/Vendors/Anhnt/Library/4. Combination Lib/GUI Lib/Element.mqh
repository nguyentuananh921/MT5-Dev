...
string CElement::IconFile(bool resource_index_mode = false) 
 {
    ...
    if (::ArraySize(m_images_group[0].m_image) < 1)
        return ("");
    // --- Return image path
    return ((!resource_index_mode) ? m_images_group[0].m_image[0].BmpPath() : (string)m_images_group[0].m_image[0].ResourceIndex());
 }
//+------------------------------------------------------------------+
...
string CElement::IconFileLocked(bool resource_index_mode = false) 
 {
  ...
  // ---If there is no image in the group
    if (::ArraySize(m_images_group[0].m_image) < 2)
        return ("");
  // --- Return image path
    return ((!resource_index_mode) ? m_images_group[0].m_image[1].BmpPath() : (string)m_images_group[0].m_image[1].ResourceIndex());
 }
//+------------------------------------------------------------------+
...
string CElement::IconFilePressed(bool resource_index_mode = false) 
 {
  ...
  // ---If there is no image in the group
    if (::ArraySize(m_images_group[0].m_image) < 3)
        return ("");
  // --- Return image path
    return ((!resource_index_mode) ? m_images_group[0].m_image[2].BmpPath() : (string)m_images_group[0].m_image[2].ResourceIndex());
 }
//+------------------------------------------------------------------+
...
string CElement::IconFilePressedLocked(bool resource_index_mode = false) 
 {
  ...
  // ---If there is no image in the group
    if (::ArraySize(m_images_group[0].m_image) < 4)
        return ("");
  // --- Return image path
    return ((!resource_index_mode) ? m_images_group[0].m_image[3].BmpPath() : (string)m_images_group[0].m_image[3].ResourceIndex());
 }
//+------------------------------------------------------------------+
//| Item Update |
//+------------------------------------------------------------------+
...
void CElement::DrawText(void) 
 {
  ...
  // --- If the element is locked
    if (m_is_locked) clr = m_label_color_locked;
    else 
     {
