//+------------------------------------------------------------------+
//|                            BarPatternControlDragonflyDoji.mqh    |
...
#ifndef __BARPATTERNCONTROLDRAGONDOJI_MQH__
#define __BARPATTERNCONTROLDRAGONDOJI_MQH__
 #property strict    // Necessary for mql4
 #include "BarPatternControlDoji.mqh"
 #include "..\..\BarSeriesPatterns\SCandlesPatterns\PatternDragonflyDoji.mqh"

 //--- Inherits Doji (body check). Adds: long lower shadow, near-zero upper shadow.
 //    m_ratio_larger_shadow_to_candle_size → min lower shadow ratio (default 0.70)
 //    m_ratio_smaller_shadow_to_candle_size→ max upper shadow ratio (default 0.05)
 #ifndef CBarPatternControlDragonflyDoji_MQH_DECLARATION
 #define CBarPatternControlDragonflyDoji_MQH_DECLARATION
  //+------------------------------------------------------------------+
  //| Dragonfly Doji control (doji + long lower shadow)                |
  //+------------------------------------------------------------------+
  class CBarPatternControlDragonflyDoji : public CBarPatternControlDoji
   {
    protected:
          ...
          virtual ulong                  GetPatternCode(const ENUM_PATTERN_DIRECTION direction, const datetime time) const
                                          {
                                            return(time + PATTERN_TYPE_DRAGONFLY_DOJI + PATTERN_STATUS_PA +
    ...
    public:
        ...
        //    param[2] double: min lower shadow ratio  (long lower shadow, default 0.70)
        //    param[3] double: max upper shadow ratio  (near-zero upper,   default 0.05)
                              CBarPatternControlDragonflyDoji(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                              CArrayObj *list_series, CArrayObj *list_patterns,
                                                              const MqlParam &param[]);
   };
 #endif // CBarPatternControlDragonflyDoji_MQH_DECLARATION
 #ifndef CBarPatternControlDragonflyDoji_MQH_IMPLEMENTATION
 #define CBarPatternControlDragonflyDoji_MQH_IMPLEMENTATION
   CBarPatternControlDragonflyDoji::CBarPatternControlDragonflyDoji(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                                     CArrayObj *list_series, CArrayObj *list_patterns,
                                                                     const MqlParam &param[]) :
   ...
   ENUM_PATTERN_DIRECTION CBarPatternControlDragonflyDoji::FindPattern(const datetime series_bar_time,
   ...
   CBarPattern *CBarPatternControlDragonflyDoji::CreatePattern(const ENUM_PATTERN_DIRECTION direction,
                                                                const uint id, CBar *bar)
     {
      if(bar == NULL) return NULL;
      MqlRates rates = {0};
      this.SetBarData(bar, rates);
      CPatternDragonflyDoji *obj = new CPatternDragonflyDoji(id, this.Symbol(), this.Timeframe(), rates, direction);
   ...
   CArrayObj *CBarPatternControlDragonflyDoji::GetListPatterns(void)
     {
      ...
      list            = CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_TYPE,                          PATTERN_TYPE_DRAGONFLY_DOJI,       EQUAL);
      return            CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_CTRL_OBJ_ID,                   this.ObjectID(),                   EQUAL);
     }
 #endif // CBarPatternControlDragonflyDoji_MQH_IMPLEMENTATION
#endif // __BARPATTERNCONTROLDRAGONDOJI_MQH__
