...
#ifndef __BARPATTERNCONTROLHAMMER_MQH__
#define __BARPATTERNCONTROLHAMMER_MQH__
 #property strict    // Necessary for mql4
 //+------------------------------------------------------------------+
 //| Include files                                                    |
 //+------------------------------------------------------------------+
 #include "..\BarPatternControl.mqh"
 #include "..\..\BarSeriesPatterns\SCandlesPatterns\PatternHammer.mqh"

 //--- Field reuse for Hammer (stored in base class protected fields):
 //    m_ratio_body_to_candle_size          → max body ratio       (small body,        default 0.35)
 //    m_ratio_larger_shadow_to_candle_size → min lower shadow ratio (long lower shadow, default 0.55)
 //    m_ratio_smaller_shadow_to_candle_size→ max upper shadow ratio (short upper shadow,default 0.10)
 #ifndef CBarPatternControlHammer_MQH_DECLARATION
 #define CBarPatternControlHammer_MQH_DECLARATION
  //+------------------------------------------------------------------+
  //| Hammer control (1-candle bullish reversal)                       |
  //+------------------------------------------------------------------+
  class CBarPatternControlHammer : public CBarPatternControl
   {
    protected:
          ...
          virtual ulong                  GetPatternCode(const ENUM_PATTERN_DIRECTION direction, const datetime time) const
                                          {
                                            return(time + PATTERN_TYPE_HAMMER + PATTERN_STATUS_PA +
    ...
    public:
        ...
        //    param[2] double: min lower shadow/candle ratio  (long lower shadow,  default 0.55)
        //    param[3] double: max upper shadow/candle ratio  (short upper shadow, default 0.10)
                              CBarPatternControlHammer(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                       CArrayObj *list_series, CArrayObj *list_patterns,
                                                       const MqlParam &param[]);
   };
 #endif // CBarPatternControlHammer_MQH_DECLARATION
 #ifndef CBarPatternControlHammer_MQH_IMPLEMENTATION
 #define CBarPatternControlHammer_MQH_IMPLEMENTATION
   //+------------------------------------------------------------------+
   //| Constructor                                                      |
   //+------------------------------------------------------------------+
   CBarPatternControlHammer::CBarPatternControlHammer(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                       CArrayObj *list_series, CArrayObj *list_patterns,
                                                       const MqlParam &param[]) :
    CBarPatternControl(symbol, timeframe, PATTERN_STATUS_PA, PATTERN_TYPE_HAMMER,
   ...
   //| Create object ID                                                 |
   //+------------------------------------------------------------------+
   ulong CBarPatternControlHammer::CreateObjectID(void)
     {
   ...
   //+------------------------------------------------------------------+
   CBarPattern *CBarPatternControlHammer::CreatePattern(const ENUM_PATTERN_DIRECTION direction,
                                                         const uint id, CBar *bar)
     {
      if(bar == NULL) return NULL;
      MqlRates rates = {0};
      this.SetBarData(bar, rates);
      CPatternHammer *obj = new CPatternHammer(id, this.Symbol(), this.Timeframe(), rates, direction);
   ...
   //+------------------------------------------------------------------+
   //| Search for Hammer: small body, long lower shadow, short upper    |
   //+------------------------------------------------------------------+
   ENUM_PATTERN_DIRECTION CBarPatternControlHammer::FindPattern(const datetime series_bar_time,
                                                                  MqlRates &mother_bar_data) const
    {
     ...
     //--- Long lower shadow
      ...
      if(list == NULL || list.Total() == 0) return WRONG_VALUE;
     //--- Short upper shadow
      list = CTimeseriesSelect::ByBarProperty(list, BAR_PROP_RATIO_UPPER_SHADOW_TO_CANDLE_SIZE, this.RatioSmallerShadowToCandleSizeValue(), EQUAL_OR_LESS);
      if(list == NULL || list.Total() == 0) return WRONG_VALUE;
      CBar *bar = list.At(0);
   ...
   //+------------------------------------------------------------------+
   //| Return list of Hammer patterns                                   |
   //+------------------------------------------------------------------+
   CArrayObj *CBarPatternControlHammer::GetListPatterns(void)
     {
      ...
      list            = CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_TYPE,                          PATTERN_TYPE_HAMMER,     EQUAL);
      return            CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_CTRL_OBJ_ID,                   this.ObjectID(),         EQUAL);
     }
 #endif // CBarPatternControlHammer_MQH_IMPLEMENTATION
#endif // __BARPATTERNCONTROLHAMMER_MQH__
