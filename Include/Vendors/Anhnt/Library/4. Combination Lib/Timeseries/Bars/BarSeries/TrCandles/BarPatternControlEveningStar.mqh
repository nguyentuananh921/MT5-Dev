//+------------------------------------------------------------------+
//|                                BarPatternControlEveningStar.mqh  |
...
#ifndef __BARPATTERNCONTROLEVENINGSTAR_MQH__
#define __BARPATTERNCONTROLEVENINGSTAR_MQH__
 #property strict    // Necessary for mql4
 //+------------------------------------------------------------------+
 //| Include files                                                    |
 //+------------------------------------------------------------------+
 #include "..\BarPatternControl.mqh"
 #include "..\..\BarSeriesPatterns\TrCandlesPatterns\PatternEveningStar.mqh"

 //--- Field reuse for Evening Star (stored in base class protected fields):
 //    m_ratio_body_to_candle_size           → min body ratio for candle 1 (large bullish, e.g. 0.60)
 ...
 #ifndef CBarPatternControlEveningStar_MQH_DECLARATION
 #define CBarPatternControlEveningStar_MQH_DECLARATION
  //+------------------------------------------------------------------+
  //| Evening Star pattern control class (3-candle bearish reversal)   |
  //+------------------------------------------------------------------+
  class CBarPatternControlEveningStar : public CBarPatternControl
   {
    protected:
        ...
        //--- (4) return the list of patterns managed by the object
          ...
          virtual ulong                  GetPatternCode(const ENUM_PATTERN_DIRECTION direction, const datetime time) const
                                          {
                                            return(time + PATTERN_TYPE_EVENING_STAR + PATTERN_STATUS_PA +
    ...
    public:
        ...
        //    param[3] double: min penetration into candle 1 body    (candle 3,       default 0.50)
                              CBarPatternControlEveningStar(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                            CArrayObj *list_series, CArrayObj *list_patterns,
                                                            const MqlParam &param[]);
   };
 #endif // CBarPatternControlEveningStar_MQH_DECLARATION
 #ifndef CBarPatternControlEveningStar_MQH_IMPLEMENTATION
 #define CBarPatternControlEveningStar_MQH_IMPLEMENTATION
   //+------------------------------------------------------------------+
   //| Constructor                                                      |
   //+------------------------------------------------------------------+
   CBarPatternControlEveningStar::CBarPatternControlEveningStar(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                                  CArrayObj *list_series, CArrayObj *list_patterns,
                                                                  const MqlParam &param[]) :
   ...
   //| Create object ID based on pattern search criteria                |
   //+------------------------------------------------------------------+
   ulong CBarPatternControlEveningStar::CreateObjectID(void)
     {
   ...
   //+------------------------------------------------------------------+
   CBarPattern *CBarPatternControlEveningStar::CreatePattern(const ENUM_PATTERN_DIRECTION direction,
                                                                 const uint id, CBar *bar)
     {
      if(bar == NULL) return NULL;
      MqlRates rates = {0};
      this.SetBarData(bar, rates);
      CPatternEveningStar *obj = new CPatternEveningStar(id, this.Symbol(), this.Timeframe(), rates, direction);
   ...
   //+------------------------------------------------------------------+
   //| Search for Evening Star on 3 consecutive bars ending at time     |
   //+------------------------------------------------------------------+
   ENUM_PATTERN_DIRECTION CBarPatternControlEveningStar::FindPattern(const datetime series_bar_time,
                                                                        MqlRates &mother_bar_data) const
    {
     //--- Get all bars up to and including series_bar_time
      ...
      int n = list.Total();
     //--- candle 3 = bearish confirmation (the bar at series_bar_time)
     ...
     //--- Condition 1: candle 1 must be bullish with a large body
     ...
     //--- Condition 3: candle 3 must be bearish
   ...
   //+------------------------------------------------------------------+
   //| Return the list of Evening Star patterns managed by this object  |
   //+------------------------------------------------------------------+
   CArrayObj *CBarPatternControlEveningStar::GetListPatterns(void)
     {
      ...
      list            = CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_TYPE,                          PATTERN_TYPE_EVENING_STAR,     EQUAL);
      return            CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_CTRL_OBJ_ID,                   this.ObjectID(),               EQUAL);
     }
 #endif // CBarPatternControlEveningStar_MQH_IMPLEMENTATION
#endif // __BARPATTERNCONTROLEVENINGSTAR_MQH__
