//+------------------------------------------------------------------+
//|                          BarPatternControlThreeWhiteSoldiers.mqh |
...
#ifndef __BARPATTERNCONTROLTHREEWHITESOLDIERS_MQH__
#define __BARPATTERNCONTROLTHREEWHITESOLDIERS_MQH__
 #property strict    // Necessary for mql4
 //+------------------------------------------------------------------+
 //| Include files                                                    |
 //+------------------------------------------------------------------+
 #include "..\BarPatternControl.mqh"
 #include "..\..\BarSeriesPatterns\TrCandlesPatterns\PatternThreeWhiteSoldiers.mqh"

 //--- Field reuse for Three White Soldiers (stored in base class protected fields):
 //    m_ratio_body_to_candle_size  → min body/candle ratio for all 3 candles (default 0.60)
 #ifndef CBarPatternControlThreeWhiteSoldiers_MQH_DECLARATION
 #define CBarPatternControlThreeWhiteSoldiers_MQH_DECLARATION
  //+------------------------------------------------------------------+
  //| Three White Soldiers control (3-candle bullish continuation)     |
  //+------------------------------------------------------------------+
  class CBarPatternControlThreeWhiteSoldiers : public CBarPatternControl
   {
    protected:
        ...
        //--- (4) return the list of patterns managed by the object
          ...
          virtual ulong                  GetPatternCode(const ENUM_PATTERN_DIRECTION direction, const datetime time) const
                                          {
                                            return(time + PATTERN_TYPE_THREE_WHITE_SOLDIERS + PATTERN_STATUS_PA +
    ...
    public:
        ...
        //    param[1] double: min body/candle ratio for all 3 candles    (default 0.60)
                              CBarPatternControlThreeWhiteSoldiers(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                                   CArrayObj *list_series, CArrayObj *list_patterns,
                                                                   const MqlParam &param[]);
   };
 #endif // CBarPatternControlThreeWhiteSoldiers_MQH_DECLARATION
 #ifndef CBarPatternControlThreeWhiteSoldiers_MQH_IMPLEMENTATION
 #define CBarPatternControlThreeWhiteSoldiers_MQH_IMPLEMENTATION
   //+------------------------------------------------------------------+
   //| Constructor                                                      |
   //+------------------------------------------------------------------+
   CBarPatternControlThreeWhiteSoldiers::CBarPatternControlThreeWhiteSoldiers(const string symbol, const ENUM_TIMEFRAMES timeframe,
                                                                               CArrayObj *list_series, CArrayObj *list_patterns,
                                                                               const MqlParam &param[]) :
    CBarPatternControl(symbol, timeframe, PATTERN_STATUS_PA, PATTERN_TYPE_THREE_WHITE_SOLDIERS,
   ...
   //| Create object ID based on pattern search criteria                |
   //+------------------------------------------------------------------+
   ulong CBarPatternControlThreeWhiteSoldiers::CreateObjectID(void)
     {
   ...
   //+------------------------------------------------------------------+
   CBarPattern *CBarPatternControlThreeWhiteSoldiers::CreatePattern(const ENUM_PATTERN_DIRECTION direction,
                                                                     const uint id, CBar *bar)
     {
      if(bar == NULL) return NULL;
      MqlRates rates = {0};
      this.SetBarData(bar, rates);
      CPatternThreeWhiteSoldiers *obj = new CPatternThreeWhiteSoldiers(id, this.Symbol(), this.Timeframe(), rates, direction);
   ...
   //+------------------------------------------------------------------+
   //| Search for Three White Soldiers pattern ending at series_bar_time|
   //+------------------------------------------------------------------+
   ENUM_PATTERN_DIRECTION CBarPatternControlThreeWhiteSoldiers::FindPattern(const datetime series_bar_time,
                                                                             MqlRates &mother_bar_data) const
    {
     ...
     //--- Condition 2: bar1 opens within bar0's body and closes higher
     //    Bullish bar: BottomBody = Open, TopBody = Close
      if(bar1.Open() < bar0.BottomBody() || bar1.Open() > bar0.TopBody()) return WRONG_VALUE;
      if(bar1.Close() <= bar0.Close())                                     return WRONG_VALUE;

     //--- Condition 3: bar2 opens within bar1's body and closes higher
   ...
   //+------------------------------------------------------------------+
   //| Return list of Three White Soldiers patterns for this object     |
   //+------------------------------------------------------------------+
   CArrayObj *CBarPatternControlThreeWhiteSoldiers::GetListPatterns(void)
     {
      ...
      list            = CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_TYPE,                          PATTERN_TYPE_THREE_WHITE_SOLDIERS,       EQUAL);
      return            CTimeseriesSelect::ByPatternProperty(list, PATTERN_PROP_CTRL_OBJ_ID,                   this.ObjectID(),                         EQUAL);
     }
 #endif // CBarPatternControlThreeWhiteSoldiers_MQH_IMPLEMENTATION
#endif // __BARPATTERNCONTROLTHREEWHITESOLDIERS_MQH__
