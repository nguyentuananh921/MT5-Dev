...
#define __PATTERNDOJI_MQH__
 ...
 #define CDOJI_MQH_DECLARATION
  ...
  //+------------------------------------------------------------------+
  class CPatternDoji : public CBarPattern
    {
      public:
          ...
          virtual string    TypeDescription(void)   const { return CMessage::Text(MSG_LIB_TEXT_PATTERN_TYPE_DOJI);                          }
                            CPatternDoji(const uint id, const string symbol, const ENUM_TIMEFRAMES timeframe,
                                   MqlRates &rates, const ENUM_PATTERN_DIRECTION direct);
    };
  #endif // CDOJI_MQH_DECLARATION
  #ifndef CDOJI_MQH_IMPLEMENTATION
  #define CDOJI_MQH_IMPLEMENTATION
   CPatternDoji::CPatternDoji(const uint id, const string symbol, const ENUM_TIMEFRAMES timeframe,
                    MqlRates &rates, const ENUM_PATTERN_DIRECTION direct) :
    CBarPattern(PATTERN_STATUS_PA, PATTERN_TYPE_DOJI, id, direct, symbol, timeframe, rates)
    {
      this.SetProperty(PATTERN_PROP_NAME,    "Doji");
      this.SetProperty(PATTERN_PROP_CANDLES, 1);
      this.m_bars_formation = (int)this.GetProperty(PATTERN_PROP_CANDLES);
    }
  #endif // CDOJI_MQH_IMPLEMENTATION
#endif // __PATTERNDOJI_MQH__
