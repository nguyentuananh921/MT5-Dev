...
#define __PATTERNHAMMER_MQH__
 ...
 #define CHAMMER_MQH_DECLARATION
  ...
  //+------------------------------------------------------------------+
  class CPatternHammer : public CBarPattern
    {
      public:
          ...
          virtual string    TypeDescription(void)   const { return CMessage::Text(MSG_LIB_TEXT_PATTERN_TYPE_HAMMER);                          }
                            CPatternHammer(const uint id, const string symbol, const ENUM_TIMEFRAMES timeframe,
                                   MqlRates &rates, const ENUM_PATTERN_DIRECTION direct);
    };
  #endif // CHAMMER_MQH_DECLARATION
  #ifndef CHAMMER_MQH_IMPLEMENTATION
  #define CHAMMER_MQH_IMPLEMENTATION
   CPatternHammer::CPatternHammer(const uint id, const string symbol, const ENUM_TIMEFRAMES timeframe,
                    MqlRates &rates, const ENUM_PATTERN_DIRECTION direct) :
    CBarPattern(PATTERN_STATUS_PA, PATTERN_TYPE_HAMMER, id, direct, symbol, timeframe, rates)
    {
      this.SetProperty(PATTERN_PROP_NAME,    "Hammer");
      this.SetProperty(PATTERN_PROP_CANDLES, 1);
      this.m_bars_formation = (int)this.GetProperty(PATTERN_PROP_CANDLES);
    }
  #endif // CHAMMER_MQH_IMPLEMENTATION
#endif // __PATTERNHAMMER_MQH__
